...
Depends on the parameterization of the framework, its interpretations, and whether to allow covriates incorporated into the mixing proportions and/or the bivariate gamma densities of the finite mixture models, a family of models are proposed, including: 
...
The first example is demonstrated using a simulated data set called `gatingsim` as illustrated in Hu et al (2019), and is availabe in the `mvClaim` package. 
...
est <- BGE(gatingsim[,1:2], verbose=FALSE, maxit=5)
summary(est)
```
...
Suppose the expert network is of type `"CC"` and the gating network is of type `"C"`, the mixture model can be fitted via: 
``` {r eval = FALSE}
m1 <- MBGC(modelName = "CC", y = c("y1","y2"), G = 2, gating = "C", data = gatingsim, maxit=5, verbose = FALSE)
summary(m1)
m2 <- MBGC(modelName = "CC", y = c("y1","y2"), G = 3, gating = "C", data = gatingsim, maxit=5, verbose = FALSE)
m3 <- MBGC(modelName = "CC", y = c("y1","y2"), G = 4, gating = "C", data = gatingsim, maxit=5, verbose = FALSE)
m4 <- MBGC(modelName = "CC", y = c("y1","y2"), G = 5, gating = "C", data = gatingsim, maxit=5, verbose = FALSE)
```
When fitting `G=1` in this case will be equivalent to fitting the `BGE` function. Model types can be set as `modelName = "CI"` or `"IC"`. Furthermore,covariates can enter the gating network with, for example `"gating = ~w1+w2+w3"`; in this case we know it is the true data generating process: 
``` {r eval = TRUE, fig.width=6, fig.height=4.5, fig.align="center"}
m5 <- MBGC(modelName = "CC", y=c("y1","y2"), G = 2, gating = ~w1+w2+w3, data = gatingsim, maxit=5, verbose = FALSE)
summary(m5)
...
The data were simulated based on a full MoE model, i.e. covariates were entering both the gating and experts networks.
The detailed simulation process can be found in Hu et al (2019). 
The pairwise plot of the data is shown below. 
...
pairs(fullsim[,1:5], # upper.panel = NULL, 
      ...
      labels = expression("Y"[1],"Y"[2],"w"[1],"w"[2],"w"[3]))
```
Now we only consider the cases when covariates are entering the expert networks.
In such cases, there are 9 different model types: `"*VC"`, `"*VI"`, `"*VV"`, `"*VE"`, `"*CV"`, `"*IV"`, `"*EV"`, `"*EC"`, `"*CE"`.
Typically model selection issue needs to be addressed if the true model is unknown. We recommend a stepwise forward selection starting from a null model, i.e. fitting one bivariate gamma distribution without covariates. 
...
m6 <- MBGR(modelName = "VV", y=c("y1","y2"),
           ...
           f3     = ~ w1 + w2 + w3,
           f4     = ~ w1 + w2 + w3,
           gating = ~ w1 + w2 + w3, maxit = 5, verbose = FALSE)
summary(m6)
plot(m6, what = "classification")
```
There is also a companying `fullsim.test` data set available in this package that has been simulated similarly for test data prediction purpose. 
Based on the fitted model, predictions on the new data can be done via:
```{r eval = FALSE}
pred <- predict(m6, newdata=fullsim.test)
```
And the predictions are plotted in the following figure. 
``` {r eval = FALSE, echo = FALSE, fig.width=4, fig.height=3, fig.align="center"}
