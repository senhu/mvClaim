...
MBGR_VE <- function(y,
  ...
  if (initialization=="mclust"){
    ...
    newdata$be          <- rowSums(( z.init*(alpha1.current+alpha2.current+alpha3.current) ) / ( z.init*rep_col(x1+x2+x3, G) ))
  ...
  while ( (loglike.diff > tol) && (j <= maxit) ) {
    ...
    for (g in c(1:G)){
      m1             <- stats::glm(alpha1.current[,g] ~ Model.Matrix.1[,-1], family=Gamma(link="log"))
      ...
      m2             <- stats::glm(alpha2.current[,g] ~ Model.Matrix.2[,-1], family=Gamma(link="log"))
    ...
    m4             <- stats::glm(beta.current ~ Model.Matrix.4[,-1], family=Gamma(link="log")  )#, weights=z.new[,g])
    coef4.new.temp <- as.vector(m4$coefficients)
    coef4.optim    <- stats::optim(par               = coef4.new.temp,
                            fn                = Q.b.function,
                            gr                = NULL,
                            hessian           = TRUE,
                            control           = list(fnscale=-1, maxit=2e+9))
  ...
  # fitted values
  if (gating=="C" || gating=="E"){
    y1.fitted <- ((alpha1.current + alpha3.current) / rep_col(beta.current, G)) %*% p.z.current
    y2.fitted <- ((alpha2.current + alpha3.current) / rep_col(beta.current, G)) %*% p.z.current
  }  else {
    y1.fitted <- rowSums(p.z.current * ((alpha1.current + alpha3.current)/ rep_col(beta.current, G) ) )
    y2.fitted <- rowSums(p.z.current * ((alpha2.current + alpha3.current)/ rep_col(beta.current, G) ) )
  }
