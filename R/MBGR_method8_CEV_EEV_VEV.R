...
MBGR8 <- function(data,
  ...
  } else {stop("l4 cannot be NULL or has to be a formula.")}
  ...
  if (start=="mclust"){
    ...
    Model.Matrix.4 <- as.matrix(Matrix::sparse.model.matrix(l4.new[-2], data=data1))
    ...
    for (gg in seq_len(G)){
      ...
      be                  <- (z.current[,gg]*(alpha1.current[,gg]+alpha2.current[,gg]+alpha3.current[,gg]) ) / ( z.current[,gg]*(x1+x2+x3) )
      data1$be            <- be
  ...
  if (start=="random"){
    stop("start should be as Mclust")
  }
  ...
  while ( (loglike.diff > tol) && (j <= maxit) ) {
    ...
    coef1.optim    <- optim(par               = coef1.new.temp, #coef1.current[,g],
                            fn                = Q.a.function,
    ...
    coef2.optim    <- optim(par               = coef2.new.temp, #coef2.current[,g],
                            fn                = Q.a.function,
    ...
    coef3.optim    <- optim(par               = coef3.new.temp, # coef3.current[,g],
                            fn                = Q.a.function,
    ...
    coef1.trace    <- rbind(coef1.trace, coef1.new)
    coef2.trace    <- rbind(coef2.trace, coef2.new)
    coef3.trace    <- rbind(coef3.trace, coef3.new)
