#------------------------------------
# Method 4: only beta is regressed on its covariates, not alpha
#------------------------------------
BGR <- function(data,
                l1,
                l2,
  ...
  if (!is.null(l4)){
    ...
    else {l4.new <- formula(paste("be", as.character(l3)[2], sep="~"))}
  ...
  alpha1.current   <- 1/summary(start.temp.glm.1)$dispersion #rep(MASS::gamma.shape(start.temp.glm.1)$alpha, n)
  alpha2.current   <- 1/summary(start.temp.glm.2)$dispersion #rep(MASS::gamma.shape(start.temp.glm.2)$alpha, n)
  alpha3.current   <- 1/summary(start.temp.glm.3)$dispersion #rep(MASS::gamma.shape(start.temp.glm.3)$alpha, n)
  ...
  while ( (loglike.diff > tol) && (j <= maxit) ) {
    ...
    # coef4.new <- m4$coefficients
    # beta.new  <- m4$fitted.values
    #--------------------------------
    # Q.b.function    <- function(coef){
    #   q.b.res <- sum(alpha1.current * log(exp(coef %*% t(Model.Matrix.4)))) + 
    #     sum(alpha2.current * log(exp(coef %*% t(Model.Matrix.4)))) +
    #     sum(alpha3.current * log(exp(coef %*% t(Model.Matrix.4)))) -
    #     sum(exp(coef %*% t(Model.Matrix.4)) * Expected.x1) -
    #     sum(exp(coef %*% t(Model.Matrix.4)) * Expected.x2) -
    #     sum(exp(coef %*% t(Model.Matrix.4)) * Expected.x3)
    #   return(q.b.res)
    # }
    Q.b.function    <- function(coef){
    ...
    coef4.new      <- optim(par     = coef4.current,
                            fn      = Q.b.function,
                            gr      = NULL,
                            hessian = TRUE,
