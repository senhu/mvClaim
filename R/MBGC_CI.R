...
MBGC <- function(data,
  ...
  if (start=="given"){
    alpha1.current=starting.value[1,]
    alpha2.current=starting.value[2,]
    alpha3.current=starting.value[3,]
    beta.current=starting.value[4,]
    p.z.current=starting.value[5,]
  }
  ...
  while ( (loglike.diff >= tol) && (j <= maxit) ) {
    ...
    pzy.trace <- append(pzy.trace, list(pzy))
    if (table(round(rowSums(pzy),0))!=n){stop("Wrong E-step estimation: pzy")}
    ...
    p.z.new    <- colSums(pzy)/n

    for (g in seq_len(G)){
      beta.new[g] <- ((alpha1.current[g]+alpha2.current[g]+alpha3.current[g])*sum(pzy[,g])) / (pzy[,g]%*%(Expected.x1[,g]+Expected.x2[,g]+Expected.x3[,g]))
      alpha1.rootfun <- function(alpha1.var,wh){
        log(beta.new[wh])*sum(pzy[,wh]) - digamma(alpha1.var)*sum(pzy[,wh]) + pzy[,wh]%*%Expected.logx1[,g]
      }
      ...
      alpha2.rootfun <- function(alpha2.var,wh){
        log(beta.new[wh])*sum(pzy[,wh]) - digamma(alpha2.var)*sum(pzy[,wh]) + pzy[,wh]%*%Expected.logx2[,g]
      }
      ...
      alpha3.rootfun <- function(alpha3.var,wh){
        log(beta.new[wh])*sum(pzy[,wh]) - digamma(alpha3.var)*sum(pzy[,wh]) + pzy[,wh]%*%Expected.logx3[,g]
      }
...
#              maxit=100, tol=1e-5,
#              start = "given",
#              starting.value = matrix(c(.5,.5,.5,1e-4,.5,1.5, 1.5, 1.5, 1e-3,.5), ncol=2, byrow=FALSE),
#              Aitken = FALSE, verbose=TRUE)
# try2$loglikelihood
# try2$BIC
# try2$alpha1
...
#              start = "given",
#              starting.value = matrix(c(2,.5,2,1e-4,1/3,.5,2,2,1e-3,1/3,.5,.5,.5,1e-3,1/3), ncol=3, byrow=FALSE),
#              Aitken = FALSE, verbose=TRUE)
# try3$BIC
# try3$loglikelihood
# table(try3$class)
