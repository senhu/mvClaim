...
MBGR6 <- function(data,
  ...
  if (start=="mclust"){
    ...
    for (gg in seq_len(G)){
      ...
      alpha1.current.temp[gg]  <- 1/summary(start.temp.glm.1)$dispersion
      alpha2.current.temp[gg]  <- 1/summary(start.temp.glm.2)$dispersion
      alpha3.current.temp[gg]  <- 1/summary(start.temp.glm.3)$dispersion
      be                  <- (alpha1.current.temp[gg]+alpha2.current.temp[gg]+alpha3.current.temp[gg]) / (x1+x2+x3)
      data1$be            <- be
  ...
  while ( (loglike.diff > tol) && (j <= maxit) ) {
    ...
    alpha2.trace   <- c(alpha2.trace, alpha2.new)
    alpha3.trace   <- c(alpha3.trace, alpha3.new)
