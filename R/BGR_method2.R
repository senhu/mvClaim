#------------------------------------
# Method 2: same beta for all observations
#------------------------------------
BGR <- function(data,
                l1,
                l2,
...
plot(cbind(naviva$COST_AD, naviva$COST_TPD), xlab="AD", ylab="PD")
points(y1.fitted, y2.fitted, col=2, pch=20)

uni.pred1 <- fitted(glm(COST_AD ~ PH_GEND + VEH_FUEL_TYPE + VEH_TRANS + NCDLAST.factor + NCD_PROT + STEPBACK + EXCESS_TOT.factor + PH_PENPTS + MD_LIC_CAT_A, 
                    family = Gamma(link="log"), data=naviva))
uni.pred2 <- fitted(glm(COST_TPD ~ PH_GEND + VEH_FUEL_TYPE + VEH_TRANS + NCDLAST.factor + NCD_PROT + STEPBACK + EXCESS_TOT.factor + PH_PENPTS + MD_LIC_CAT_A, 
                    family = Gamma(link="log"), data=naviva))
points(uni.pred1, uni.pred2, col=2, pch=20)
